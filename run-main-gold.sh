...
model_dir_s3=${model_dir_s2}/gold
[ ! -d ${model_dir_s1} ] && mkdir -p ${model_dir_s1}
[ ! -d ${model_dir_s2} ] && mkdir -p ${model_dir_s2}
[ ! -d ${model_dir_s3} ] && mkdir -p ${model_dir_s3}

s1_oro_langs="none"
s2_oro_langs="none"
if [[ $s1_oro_langs == "none" && $s2_oro_langs == "none"]]; then
     model_init_dir=${model_dir_s2}/${tgt}_best.model
fi 
...
CUDA_LAUNCH_BLOCKING=1 python3 /home/sli136/silver-data-creation/run.py \
    --batch_size 4 \
    --phrase \
    --stage1_oro_langs none --stage1_oro_size 20000 \
    --stage2_oro_langs none --stage2_oro_size 20000 \
    --stage2_silver_langs none --stage2_silver_size -1 \
    --tgt_lang ${tgt} \
